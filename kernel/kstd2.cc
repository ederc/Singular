...
struct LabeledPoly
{
	...
	static LabeledPoly* CreateOneSig(const poly p, const int idx)
	{		
		poly p_copy = pCopy(p);
		pNorm(p_copy);
    poly pSig = pOne();
    p_SetComp (pSig, idx+1, currRing);
    p_ExpVectorAdd (pSig, p, currRing);
		return new LabeledPoly(p_copy, pSig);
	}
	
	static LabeledPoly* CreateZeroPoly(const poly having_hm_sig, const int idx, const poly new_p)
	{
    poly pSig = pLmInit (new_p);
    p_SetComp (pSig, idx+1, currRing);
    p_ExpVectorAdd (pSig, having_hm_sig, currRing);
		return new LabeledPoly(NULL, pSig);
	}
